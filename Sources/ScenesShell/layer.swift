...
class bgLayer : RenderableEntity {
    ...
    func gRow(canvas:Canvas, obj: Image, columns: Int, yt: Int) {
        for i in 0 ... columns {        
            obj.renderMode = .destinationRect(Rect(topLeft:Point(x:i * csx / 21, y:ys + yt * csy / 11 - csy / 11), size:Size(width:csx / 21 + 1, height:csy / 11 + 1)))
            canvas.render(obj)
            xs = i * csx / 21
        }
    }
    func gGrid(canvas:Canvas, obj: Image, columns: Int, yt: Int, xt: Int, rows: Int) {
        for i in 0 ... rows {
            ...
            obj.renderMode = .destinationRect(Rect(topLeft:Point(x:xs + xt * csx / 21, y:i * csy / 11), size:Size(width:csx / 21, height:csy / 11)))
            ys = i * csy / 11
    ...
    override func render(canvas:Canvas) {
        ...
        if grass.isReady {
            ...
            gGrid(canvas:canvas, obj:grass, columns: 0, yt: 2, xt: 20, rows: 10)
        }
    }
}
